    ...
    <div class="ticket-card">
        ...
        <div class="ticket-layout">
            @php
                ...
                $ticketStatus = strtolower((string) ($ticket->status ?? 'paid'));
            ...
            <div class="ticket-details">
                ...
                <div class="row"><span>Status</span><strong class="status-pill {{ $isCancelled ? 'cancelled' : 'paid' }}">{{ $isCancelled ? 'CANCELLED' : 'PAID' }}</strong></div>
            ...
            <div class="qr-wrap {{ $isCancelled ? 'cancelled' : '' }}">
                <img src="{{ $ticket->qr_url }}" alt="Ticket QR Code" class="{{ $isCancelled ? 'blurred-qr' : '' }}">
                @if($isCancelled)
                    <div class="cancel-overlay">CANCELLED</div>
                @endif
            </div>
        </div>
...
.status-pill.paid { color: #98f0bc; border: 1px solid rgba(152, 240, 188, 0.4); background: rgba(152, 240, 188, 0.12); }
.status-pill.cancelled { color: #ffcbcb; border: 1px solid rgba(255, 126, 126, 0.45); background: rgba(255, 126, 126, 0.14); }
.qr-wrap {
    border: 1px solid rgba(209, 106, 255, 0.35);
    border-radius: 14px;
    display: grid;
...
.blurred-qr { filter: blur(8px) grayscale(1); }
.qr-wrap.cancelled { border-color: rgba(255, 126, 126, 0.5); }
.cancel-overlay {
    position: absolute;
    inset: 0;
    display: grid;
