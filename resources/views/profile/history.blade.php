...
<style>
	...
	.ticket-image.cancelled img {
		filter: blur(8px) grayscale(1);
	}

	.ticket-image.cancelled::after {
		content: 'CANCELLED';
		position: absolute;
		inset: 8px;
		display: grid;
	...
	.ticket-status.paid {
		color: #8df4b2;
		border-color: rgba(141, 244, 178, 0.35);
		background: rgba(141, 244, 178, 0.12);
	}
	...
	.flash-message.success {
		border: 1px solid rgba(80, 214, 150, 0.35);
		background: rgba(80, 214, 150, 0.12);
		color: #c5f6dd;
	}
...
<section class="history-page">
	<div class="history-shell">
		...
		@if(session('ticket_success'))
			<div class="flash-message success">{{ session('ticket_success') }}</div>
		@endif
		@if(session('ticket_error'))
			<div class="flash-message error">{{ session('ticket_error') }}</div>
		@endif
		...
		<div class="history-list">
			@forelse($tickets as $ticket)
						...
						<div class="ticket-card-inner">
							<div class="ticket-main">
								...
								<div class="ticket-actions">
									@php $ticketStatus = strtolower((string) $ticket->status); @endphp
									<span class="ticket-status {{ $ticketStatus === 'cancelled' ? 'cancelled' : 'paid' }}">
										{{ $ticketStatus === 'cancelled' ? 'Cancelled' : 'Paid' }}
									</span>
									<span class="cancel-note">
										Cancel before {{ $ticket->cancel_deadline->format('d M Y, h:i A') }}
									</span>
								</div>
							</div>
							...
							<div class="ticket-side">
								<div>
									...
									@endphp
									<div class="ticket-image {{ $ticketStatus === 'cancelled' ? 'cancelled' : '' }}">
				...
				@if($ticket->can_cancel)
					<form method="POST" action="{{ route('bookings.cancel', $ticket) }}" class="cancel-form" onsubmit="return confirm('Cancel this ticket? This action cannot be undone.');">
						@csrf
