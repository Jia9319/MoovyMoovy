    ...
    <div class="dashboard-wrapper">
        ...
        <main class="main-canvas">
            <div class="stats-layout">
                <div class="stat-card">
                    <h2>12</h2>
                    <p style="color: var(--text-gray); font-size: 11px; font-weight: 600; margin-top: 8px;">TICKETS</p>
                </div>
                <div class="stat-card">
                    <h2>28</h2>
                ...
                <div class="stat-card">
                    <h2>{{ $stats['saved'] }}</h2>
                    <p style="color: var(--text-gray); font-size: 11px; font-weight: 600; margin-top: 8px;">SAVED</p>
                </div>
                <div class="stat-card">
                    <h2>{{ $stats['rating'] }}</h2>
            ...
            <div id="recent-activity" style="margin-bottom: 50px;">
                <div class="section-header">
                    ...
                    <a href="#" class="view-link"
                        style="color: var(--moovy-purple); text-decoration: none; font-size: 13px; font-weight: 600;">View
                        All</a>
                </div>
                <div class="activity-list">
                    @forelse($recentActivities as $activity)
                        <div class="activity-item">
                            ...
                            <div style="flex: 1;">
                                <div style="font-size: 15px; font-weight: 600;">{{ $activity->description }}</div>
                                <div style="font-size: 12px; color: var(--text-gray); margin-top: 2px;">
                                    {{ $activity->created_at->format('M d, Y') }} • {{ $activity->location }}
                                </div>
                            </div>
                    ...
                    @empty
                        <p style="color: var(--text-gray); font-size: 14px;">No recent activities found.</p>
                    @endforelse
                </div>
            </div>
            ...
            <div id="watchlist-section">
                ...
                <div class="watchlist-grid">
                    @foreach($watchlistMovies as $movie)
                        <div class="movie-poster-wrapper">
                            <div class="poster-container">
                                <img src="{{ $movie->poster ? asset('storage/' . $movie->poster) : asset('images/no-poster.jpg') }}"
                                    class="poster-image">
                            </div>
                    ...
                    <div class="movie-poster-wrapper" onclick="toggleModal(true)">
                        <div class="poster-container"
                            style="border: 2px dashed #3d2b52; background: rgba(209, 106, 255, 0.02); display: flex; flex-direction: column; align-items: center; justify-content: center; cursor: pointer;">
                            <div style="font-size: 32px; color: var(--moovy-purple);">+</div>
                            <div
                                style="font-size: 12px; font-weight: 800; color: var(--moovy-purple); margin-top: 8px; text-transform: uppercase;">
                                Add More</div>
                        </div>
                    </div>
                </div>
            ...
            <div id="movieModal" class="modal-overlay" onclick="if(event.target == this) toggleModal(false)">
                <div class="modal-content">
                    <div class="modal-header">
                        <span class="section-title">Manage Watchlist</span>
                        <span style="cursor: pointer; font-size: 30px; color: var(--text-gray); line-height: 1;"
                            onclick="toggleModal(false)">&times;</span>
                    </div>
                    <div class="modal-scroll-area" id="modalList"
                        style="overflow-y: auto; flex-grow: 1; padding-right: 5px;">
                        <p style="text-align: center; color: var(--text-gray); padding: 20px;">Fetching movies...</p>
                    </div>
                </div>
            </div>
        </main>
